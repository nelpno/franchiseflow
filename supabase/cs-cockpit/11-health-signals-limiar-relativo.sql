-- 11 — get_franchise_health_signals: o limiar de queda passa a ser RELATIVO A REDE
--
-- POR QUE
-- O sinal "Faturamento -X%" disparava com gatilho FIXO em -10%. Medido em 08/09/2026, a
-- mediana da rede era -10,3% — ou seja, o gatilho estava colado na mediana e, por
-- construcao, METADE da rede cruzava o limiar todo dia. Nao por estar mal: por ser a
-- metade de baixo de qualquer distribuicao.
--
-- O efeito no mural: 23 das 46 unidades comparaveis com a bandeira, e 55 das 67 (82%) da
-- rede marcadas como critica ou atencao. Um radar em que 82% pisca nao prioriza nada — e a
-- mesma licao ja escrita no CLAUDE.md sobre a sentinela: alarme diario treina a pessoa a
-- ignorar o painel inteiro.
--
-- A queda em si e REAL e continua sendo mostrada: das 46 comparaveis, 31 caem e 15 sobem, e
-- a rede somada caiu 4,3% (R$ 342.822 contra R$ 358.204). O que muda e o que merece BANDEIRA:
-- cair 12% num mes em que a rede caiu 10% e desempenho mediano, nao alerta.
--
-- O QUE MUDA
-- 1. CTE `net_delta` + `corte_queda`: o gatilho vira `least(-10, mediana - 15)` e a
--    severidade alta vira `least(-30, mediana - 30)`. Num mes normal (mediana ~0) o
--    comportamento e IDENTICO ao de hoje, porque o `least` mantem o piso fixo; num mes em
--    que a rede inteira cai, so acende quem cai muito mais que os pares.
-- 2. O rotulo passa a carregar a mediana: "Faturamento -48.6% (rede -10.3%)". Sem isso o
--    Celso nao tem como saber se e a unidade ou o mercado.
--
-- Guarda de amostra: com menos de 10 unidades comparaveis a mediana e instavel, entao cai
-- no gatilho fixo de antes (-10 / -30).
--
-- Nao usa o `net.med` que a funcao ja calculava: aquele sai do CTE `growth`, que aceita
-- revprev>0, entao unidades com faturamento anterior minusculo entram e distorcem. Medido:
-- net.med = -4,4% contra -10,3% da mediana das comparaveis (mesmo criterio do proprio delta).
--
-- IMPACTO MEDIDO (simulado antes de aplicar, com os dados de 08/09/2026)
--   flag revenue_drop : 23 -> 14 unidades
--   severidade high   : 11 -> 7 unidades
--
-- NENHUM outro sinal foi tocado: giro_baixo, marketing_late, cs_agreements, cooldown,
-- parked_until, bot_silent, pix_missing e todos os demais estao byte a byte como estavam.
-- O ponto de partida deste arquivo e o 10-*.sql, que e o `pg_get_functiondef` da funcao viva
-- (paridade provada: md5 6e61dfebfccdaee4fd7dde9cd763ff21, 13548 bytes).
--
-- ROLLBACK
--   Reaplicar supabase/cs-cockpit/10-health-signals-PRODUCAO-2026-09-08.sql.
--   Nenhuma tabela, coluna ou permissao e tocada aqui — so o corpo da funcao.
--
-- PARIDADE DEPOIS DE APLICADO (conferido em 08/09/2026)
--   md5(prosrc) = a44d721a8a7913a5bea1cfed59563819
--   length(prosrc) = 14223
--
-- PINO DE NAO-REGRESSAO (rodado antes de aplicar, com a funcao antiga clonada como
-- _health_antes e as duas comparadas na MESMA query, comparando o CONJUNTO de flags e nao
-- a sequencia do jsonb_agg):
--   unidades .............. 67 / 67
--   conjunto de flags mudou em 9 unidades — e nas 9 a UNICA diferenca e revenue_drop ter saido
--   sinal NOVO em alguma unidade . (nenhum)
--   tiers antes  . 18 criticas / 37 atencao / 11 saudaveis / 1 dormente
--   tiers depois . 14 criticas / 37 atencao / 15 saudaveis / 1 dormente
--   rotulo ....... "Faturamento -62.0% (rede -10.3%)"
--
-- ⚠️ Comentario `--` so neste cabecalho, NUNCA dentro do corpo delimitado: ele some no
