...
import { BotConversation } from "@/entities/all";
...
export default function BotPerformanceCard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const conversations = await BotConversation.filter(
        { franchise_id: evoId },
        "-started_at",
        500
      );
      ...
      const converted = monthly.filter((c) => c.outcome === "converted").length;
      ...
      const tip = topAbandon ? ABANDON_TIPS[topAbandon] : null;

      setData({ total, converted, escalated, topTopics, tip });
    } catch (err) {
  ...
  const { total, converted, escalated, topTopics, tip } = data;
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div className="bg-[#16a34a]/10 rounded-xl p-3 text-center">
            ...
            <p className="text-xs text-[#7a6d6d] mt-0.5">Vendas pelo bot</p>
          </div>
        </div>
        ...
        {escalated > 0 && (
          <p className="text-xs text-[#4a3d3d] mb-3 flex items-center gap-1">
            <MaterialIcon icon="support_agent" size={14} className="text-[#7a6d6d]" />
            Precisaram da sua ajuda:{" "}
