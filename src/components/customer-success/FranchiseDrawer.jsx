...
const EVENT_LABEL = {
  contact: "Contato registrado", meeting: "Reunião marcada", resolve: "Resolvido",
  reopen: "Reaberto", note: "Nota", move: "Movido de coluna",
...
// Drawer com 3 modos:
//  - cartão de franquia  (task + row): raio-x + ações do cartão + histórico
//  - cartão geral        (task, sem row): título/descrição + ações + histórico
//  - preview do Radar     (row, sem task): raio-x + "Criar cartão" (sem histórico/ações)
export default function FranchiseDrawer({ task, row, userId, isAdmin = false, onClose, onChanged, onCreateCard }) {
  ...
  useEffect(() => {
    setNote(""); setMeetingDate(""); setEditingId(null); setConfirmId(null);
    loadEvents();
  }, [taskId, loadEvents]);

  const afterAction = async (msg) => { toast.success(msg); setNote(""); await loadEvents(); onChanged?.(); };
  ...
  const registerContact = async () => {
    ...
    try {
      ...
      await afterAction("Contato registrado");
  ...
  const resolve = async () => {
    ...
    try {
      ...
      await afterAction("Cartão resolvido");
    } catch (e) { console.error(e); toast.error(safeErrorMessage(e, "Não foi possível salvar.")); }
    finally { setSaving(false); }
  };
  ...
  const justNote = async () => {
    ...
    setSaving(true);
    try { await addCsTaskEvent(taskId, "note", note, userId, fid); await afterAction("Nota salva"); }
    catch (e) { console.error(e); toast.error(safeErrorMessage(e, "Não foi possível salvar.")); }
    finally { setSaving(false); }
  };
          ...
          <div className="space-y-5">
            ...
            {isPreview && (
              <div className="border-t border-[#291715]/5 pt-4">
                <Button onClick={() => onCreateCard?.(row)} className="w-full bg-[#b91c1c] hover:bg-[#991b1b] text-white gap-1">
            ...
            {task && (
              <div className="border-t border-[#291715]/5 pt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  <Button size="sm" variant="outline" disabled={saving} onClick={markMeeting} className="gap-1">
                  ...
                  {note.trim() && (
                    <Button size="sm" variant="ghost" disabled={saving} onClick={justNote} className="gap-1">
                      <MaterialIcon icon="note_add" size={16} /> Só anotar
                    </Button>
                  )}
                </div>
                <p className="text-[11px] text-[#8a7e7e] mt-2 leading-snug">
                  Os registros ficam só aqui (não vão pro franqueado). <b>Resolver</b> fecha <b>este cartão</b>.
