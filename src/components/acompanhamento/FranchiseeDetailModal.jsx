...
const ITEM_LABELS = {
  ...
  m2: "Responder todos os contatos novos",
  ...
  md1: "Follow-up para interessados",
  md2: "Selecionar 5-10 clientes sumidos",
  ...
  md7: "Enviar mensagem para clientes fiéis",
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl">
            ...
            <span className="text-[#4a3d3d] font-normal text-base ml-2">· {franchise.city}</span>
          </DialogTitle>
        </DialogHeader>

        {/* Calendar 30 days */}
        <div>
          <h3 className="font-semibold text-[#1b1c1d] mb-3">Calendário dos últimos 30 dias</h3>
          ...
          <div className="flex gap-4 mt-2 text-xs text-[#4a3d3d]">
        ...
        {/* Weekly bars */}
        <div>
          <h3 className="font-semibold text-[#1b1c1d] mb-3">Aderência semanal (últimas 4 semanas)</h3>
          <div className="flex gap-3 items-end h-24">
            {weeklyAdherence.map(week => (
              <div key={week.label} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-xs font-semibold text-[#1b1c1d]">{week.adherencia}%</span>
                <div className="w-full bg-[#fbf9fa] rounded-t-md" style={{ height: "60px", display: "flex", alignItems: "flex-end" }}>
                  <div
        ...
        {/* Today's items */}
        <div>
          <h3 className="font-semibold text-[#1b1c1d] mb-3">
            ...
            <div className="grid grid-cols-1 gap-1 max-h-48 overflow-y-auto">
              {DAILY_KEYS.map(key => (
                <div key={key} className="flex items-center gap-2 text-sm py-1 border-b border-[#fbf9fa]">
                  ...
                  <span className={todayItems[key] ? "text-[#1b1c1d]" : "text-[#857372]"}>
                    {ITEM_LABELS[key] || key}
                  </span>
                </div>
              ))}
