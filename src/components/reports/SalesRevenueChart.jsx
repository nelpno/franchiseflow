...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-[#291715]/10 rounded-xl shadow-lg p-3 text-sm">
      ...
      {payload.map((entry, i) => (
        <div key={i} className="flex items-center gap-2 text-[#4a3d3d]">
...
export default function SalesRevenueChart({ sales, isLoading, startDate, endDate }) {
  const getChartData = () => {
    ...
    return days.map(day => {
      ...
      const totalRevenue = daySales.reduce((sum, s) => sum + (parseFloat(s.value) || 0) - (parseFloat(s.discount_amount) || 0) + (parseFloat(s.delivery_fee) || 0), 0);
