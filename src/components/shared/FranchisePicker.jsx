...
export default function FranchisePicker({ franchises = [], title = "Selecione a unidade" }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      <MaterialIcon icon="storefront" size={48} className="text-ink-4 mb-4" />
      <h3 className="text-lg font-medium text-ink mb-1 font-plus-jakarta">{title}</h3>
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3 justify-center w-full max-w-md">
        {franchises.map((f) => (
          <button
            ...
            className="flex items-center justify-center gap-2 px-5 py-3 min-h-[48px] rounded-xl bg-white border-2 border-brand/20 text-brand font-bold text-sm hover:border-brand active:scale-[0.98] transition-all"
          >
